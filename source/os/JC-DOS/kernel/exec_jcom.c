...
#define JC_EXEC_TIER_AUTO 0xFFu

static jc_error_t jc_exec_jcom_read_fully(jc_fs_handle *handle,
                                          jc_u8 *buf,
                                          jc_u32 size) {
  jc_u32 total = 0;
  while (total < size) {
    jc_u32 chunk = 0;
    jc_error_t err = jc_fs_read(handle, buf + total, size - total, &chunk);
    if (err != JC_E_OK) {
      return err;
    }
    if (chunk == 0) {
...
static jc_error_t jc_exec_jcom_load_internal(const char *name,
  ...
  err = jc_exec_jcom_read_fully(&handle, (jc_u8 *)&header, sizeof(header));
  if (err != JC_E_OK) {
    return err;
  }
  ...
  capset = jc_dos_capset();
  tier = effective_tier;
  if (tier == JC_EXEC_TIER_AUTO && capset) {
    tier = capset->presented_cpu_tier;
  }
  if (tier != JC_EXEC_TIER_AUTO && header.min_cpu_tier > tier) {
    return JC_E_EXEC_UNSUPPORTED_TIER;
  }

  tpa_base = jc_dos_tpa_base();
  tpa_size = jc_dos_tpa_size();
  if (tpa_base == 0u || tpa_size == 0u) {
    return JC_E_EXEC_BAD_VERSION;
  }
  if (tpa_base > 0xFFFFFFFFu - tpa_size) {
    return JC_E_EXEC_BAD_VERSION;
  }
  tpa_end = tpa_base + tpa_size;
  if (header.load_addr.lo < tpa_base) {
    return JC_E_EXEC_BAD_VERSION;
  }
  if (header.load_addr.lo > 0xFFFFFFFFu - header.image_size) {
    return JC_E_EXEC_BAD_VERSION;
  }
  if (header.load_addr.lo + header.image_size > tpa_end) {
    return JC_E_EXEC_BAD_VERSION;
  }
  if (header.bss_size > 0xFFFFFFFFu - header.image_size) {
    return JC_E_EXEC_BAD_VERSION;
  }
  if (header.load_addr.lo + header.image_size + header.bss_size > tpa_end) {
    return JC_E_EXEC_BAD_VERSION;
  }
  ...
  jc_dos_memcpy(&crc_header, &header, sizeof(header));
  crc_header.crc32 = 0u;
  crc = jc_dos_crc32_update(crc, (const jc_u8 *)&crc_header,
                            header.header_size);
  ...
  while (remaining > 0) {
    ...
    crc = jc_dos_crc32_update(crc, buffer, got);
    if (offset < header.image_size) {
      jc_u32 copy_len = got;
      if (offset + copy_len > header.image_size) {
        copy_len = header.image_size - offset;
      }
      jc_dos_memcpy(load_ptr + offset, buffer, copy_len);
    }
    offset += got;
    remaining -= got;
  }

  crc = jc_dos_crc32_finalize(crc);
  if (crc != header.crc32) {
    return JC_E_EXEC_BAD_CRC;
  }

  jc_dos_memset(load_ptr + header.image_size, 0, header.bss_size);
...
jc_error_t jc_exec_jcom_load(const char *name, jc_jcom_image *out_image) {
  return jc_exec_jcom_load_internal(name, JC_EXEC_TIER_AUTO, out_image);
}

jc_error_t jc_exec_jcom_load_for_tier(const char *name,
                                      jc_u8 effective_tier,
                                      jc_jcom_image *out_image) {
  return jc_exec_jcom_load_internal(name, effective_tier, out_image);
}

jc_error_t jc_exec_jcom_run(const char *name, jc_u8 *exit_code) {
  return jc_exec_jcom_run_for_tier(name, JC_EXEC_TIER_AUTO, exit_code);
}
...
jc_error_t jc_exec_jcom_run_for_tier(const char *name,
                                     ...
                                     jc_u8 *exit_code) {
  jc_jcom_image image;
  jc_error_t err = jc_exec_jcom_load_internal(name, effective_tier, &image);
  jc_u8 rc = 0;
  jc_u8 (*entry)(void);
