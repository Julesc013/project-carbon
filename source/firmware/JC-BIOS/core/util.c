...
static char jc_to_upper(char c) {
  if (c >= 'a' && c <= 'z') {
    return (char)(c - 'a' + 'A');
  }
  return c;
...
int jc_strcasecmp(const char *a, const char *b) {
  ...
  while (a[i] && b[i]) {
    const char ca = jc_to_upper(a[i]);
    const char cb = jc_to_upper(b[i]);
    if (ca != cb) {
      return (ca < cb) ? -1 : 1;
    }
    i++;
...
static int jc_hex_val(char c, jc_u32 *out) {
  if (c >= '0' && c <= '9') {
    *out = (jc_u32)(c - '0');
    return 1;
  }
...
int jc_parse_u32(const char *s, jc_u32 *out) {
  ...
  while (jc_is_space(s[i])) {
    i++;
  }

  if (s[i] == '0' && (s[i + 1] == 'x' || s[i + 1] == 'X')) {
    i += 2;
    while (jc_hex_val(s[i], &digit)) {
      value = (value << 4) | digit;
      i++;
      saw_digit = 1;
    }
...
jc_u32 jc_crc32(const jc_u8 *data, jc_u32 len) {
  return jc_crc32_finalize(jc_crc32_update(jc_crc32_init(), data, len));
}
