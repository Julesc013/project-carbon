...
static int jc_fs_parse_name(const char *name_in,
  ...
  while (name_in[i] != '\0' && jc_is_space(name_in[i])) {
    i++;
  }
  if (name_in[i] == '\0') {
    return 0;
  ...
  if (name_len == 8 &&
      name_in[i] != '\0' && name_in[i] != '.' && !jc_is_space(name_in[i])) {
    return 0;
  }
  while (name_len < 8) {
  ...
  if (seen_dot) {
    ...
    if (ext_len == 3 &&
        name_in[i] != '\0' && !jc_is_space(name_in[i])) {
      return 0;
    }
  }
  while (ext_len < 3) {
...
static jc_error_t jc_fs_load_directory(void) {
  ...
  for (i = 0; i < g_fs.dir_sectors_512; ++i) {
    ...
    if (i * 512u < g_fs.dir_bytes) {
      ...
      jc_memcpy(dir_bytes + i * 512u, g_fs_sector_buf, to_copy);
    }
  }
  return JC_E_OK;
}
...
static jc_error_t jc_fs_flush_directory(void) {
  ...
  for (i = 0; i < g_fs.dir_sectors_512; ++i) {
    ...
    jc_u32 remaining = 0;
    jc_u32 to_copy = 0;
    jc_memset(g_fs_sector_buf, 0, sizeof(g_fs_sector_buf));
    if (offset < g_fs.dir_bytes) {
      remaining = g_fs.dir_bytes - offset;
      to_copy = remaining > 512u ? 512u : remaining;
      jc_memcpy(g_fs_sector_buf, dir_bytes + offset, to_copy);
    }
    {
...
static jc_cpmx_dir_entry_v1 *jc_fs_alloc_entry(const jc_u8 name[8],
  ...
  for (i = 0; i < g_fs.super.dir_entry_count; ++i) {
    ...
    if (entry->user == JC_CPMX_USER_FREE) {
      ...
      entry->user = 0;
      jc_memcpy(entry->name, name, 8);
      jc_memcpy(entry->ext, ext, 3);
      entry->extent = extent;
      g_fs.dir_dirty = 1;
      return entry;
    }
...
jc_error_t jc_fs_mount(void) {
  ...
  jc_arena_init(&g_fs_arena, g_fs_arena_buf, sizeof(g_fs_arena_buf));
  jc_memset(&g_fs, 0, sizeof(g_fs));
  ...
  jc_memcpy(&g_fs.super, g_fs_sector_buf, sizeof(g_fs.super));
...

  {
    ...
    temp.crc32 = 0;
    if (jc_crc32((const jc_u8 *)&temp, sizeof(temp)) != crc_expected) {
      return JC_E_FS_BAD_SUPER;
    }
  }

  g_fs.dir_entries = (jc_cpmx_dir_entry_v1 *)jc_arena_alloc(
      &g_fs_arena,
  ...
  g_fs.alloc_map = (jc_u8 *)jc_arena_alloc(&g_fs_arena,
...
jc_error_t jc_fs_list(jc_fs_list_cb cb, void *ctx) {
  ...
  for (i = 0; i < g_fs.super.dir_entry_count; ++i) {
    ...
    jc_memcpy(info.name, entry->name, 8);
    jc_memcpy(info.ext, entry->ext, 3);
...
jc_error_t jc_fs_open(jc_fs_handle *handle, const char *name, jc_u8 mode) {
  ...
  jc_memset(handle, 0, sizeof(*handle));
  handle->mode = mode;
  jc_memcpy(handle->name, name_buf, 8);
  jc_memcpy(handle->ext, ext_buf, 3);
  handle->pos = 0;
...
jc_error_t jc_fs_write(jc_fs_handle *handle, const jc_u8 *buf, jc_u32 len,
  ...
  while (remaining > 0) {
...

    {
      ...
      extent_bytes = block_in_extent * g_fs.block_size + block_offset + to_copy;
      max_records = (g_fs.block_size * 8u) / 128u;
...
jc_error_t jc_fs_rename(const char *old_name, const char *new_name) {
  ...
  for (i = 0; i < g_fs.super.dir_entry_count; ++i) {
    ...
    jc_memcpy(entry->name, new_buf, 8);
    jc_memcpy(entry->ext, new_ext, 3);
    g_fs.dir_dirty = 1;
    found = 1;
  }
  if (!found) {
