...
static jc_idepio_dma_dev g_dev;
...
static jc_u8 jc_idepio_status(jc_idepio_dma_dev *dev) {
  return jc_io_read8(dev->io_base, JC_IDEPIO_REG_STATUS);
}

static jc_error_t jc_idepio_wait_ready(jc_idepio_dma_dev *dev,
                                       jc_u64 deadline) {
...
static jc_error_t jc_idepio_wait_drq(jc_idepio_dma_dev *dev,
                                     jc_u64 deadline) {
  while (!jc_timer_expired(deadline)) {
    jc_u8 status = jc_idepio_status(dev);
    if (status & JC_IDEPIO_STATUS_ERR_MASK) {
...
static void jc_idepio_select_lba(jc_idepio_dma_dev *dev, jc_u32 lba) {
  jc_u8 drive_head = (jc_u8)(0xE0u | ((lba >> 24) & 0x0Fu));
  jc_io_write8(dev->io_base, JC_IDEPIO_REG_DRIVE_HEAD, drive_head);
}

static jc_error_t jc_idepio_identify(jc_idepio_dma_dev *dev) {
  jc_u8 data[512];
  jc_u32 i;
  jc_error_t err;
...
jc_error_t jc_blk_open(jc_u16 instance) {
  ...
  const jc_bdt_entry_v1 *dma = 0;
  jc_error_t err;

  if (JC_CFG_ENABLE_STORAGE_DMA == 0) {
    return JC_E_DEV_UNSUPPORTED;
  }
  if (g_dev.open) {
    return JC_E_OK;
  ...
  dma = jc_bdt_find(bdt, JC_DEVCLASS_DMA, 0);
  if (!dma) {
    return JC_E_DEV_UNSUPPORTED;
  }
...
static jc_error_t jc_idepio_pio_read(jc_idepio_dma_dev *dev,
                                     jc_u32 lba,
                                     jc_u8 *buf,
                                     jc_u16 count) {
  jc_u16 sector;
...
static jc_error_t jc_idepio_pio_write(jc_idepio_dma_dev *dev,
                                      jc_u32 lba,
                                      const jc_u8 *buf,
                                      jc_u16 count) {
  jc_u16 sector;
...
jc_error_t jc_blk_read(jc_u32 lba, jc_u8 *buf, jc_u16 count512) {
  ...
  jc_dma_sync_for_device(buf, (jc_u32)count512 * 512u);
  while (remaining > 0) {
  ...
  jc_dma_sync_for_cpu(buf, (jc_u32)count512 * 512u);
  return JC_E_OK;
}
