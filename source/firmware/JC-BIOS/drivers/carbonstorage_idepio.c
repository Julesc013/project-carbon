...
static jc_u8 jc_idepio_status(jc_idepio_dev *dev) {
  return jc_io_read8(dev->io_base, JC_IDEPIO_REG_STATUS);
}

static jc_error_t jc_idepio_wait_ready(jc_idepio_dev *dev, jc_u64 deadline) {
...
static jc_error_t jc_idepio_wait_drq(jc_idepio_dev *dev, jc_u64 deadline) {
  while (!jc_timer_expired(deadline)) {
    jc_u8 status = jc_idepio_status(dev);
    if (status & JC_IDEPIO_STATUS_ERR_MASK) {
      return JC_E_DEV_IO_ERROR;
...
static void jc_idepio_select_lba(jc_idepio_dev *dev, jc_u32 lba) {
  jc_u8 drive_head = (jc_u8)(0xE0u | ((lba >> 24) & 0x0Fu));
  jc_io_write8(dev->io_base, JC_IDEPIO_REG_DRIVE_HEAD, drive_head);
}

static jc_error_t jc_idepio_identify(jc_idepio_dev *dev) {
  jc_u8 data[512];
  jc_u32 i;
  jc_error_t err;
...
static jc_error_t jc_idepio_pio_read(jc_idepio_dev *dev,
                                      jc_u32 lba,
                                      jc_u8 *buf,
                                      jc_u16 count) {
  jc_u16 sector;
...
static jc_error_t jc_idepio_pio_write(jc_idepio_dev *dev,
                                       jc_u32 lba,
                                       const jc_u8 *buf,
                                       jc_u16 count) {
  jc_u16 sector;
