...
;
; SCC  2019-05-04 : v1.0.0  Modified Z80 BIOS for Z180 CPU v1.2
;                           Added support for SC111 CPU with SC119 memory
; SCC  2019-06-27 : v1.0.1  Added support for SC126 motherboard
...
;==================================================================================
            ...
            .PROC Z180          ;<SCC> SCWorkshop select processor
            .HEXBYTES 0x18      ;<SCC> SCWorkshop Intel Hex output format


; <SCC> Define one of these to select which build is required 
;#DEFINE    S5_Original         ;SC111 Z180Base=0x40 original S5 release
;#DEFINE    SC111_CF64          ;SC111 Z180Base=0xC0 CF=64MB
;#DEFINE    SC111_CF128         ;SC111 Z180Base=0xC0 CF=128MB
;#DEFINE    SC126_CF64          ;SC111 Z180Base=0xC0 CF=64MB
#DEFINE     SC126_CF128         ;SC111 Z180Base=0xC0 CF=128MB
;
; Version         Bus   MHz      I/O    Description
; =======         ===   ===      ===    ===========
; S5_Original     RC    18.432   0x40   SCM S5
; SC111_CF64      RC    18.432   0xC0   SC111 CF64
; SC111_CF128     RC    18.432   0xC0   SC111 CF128
; SC126_CF64      RC    18.432   0xC0   SC126 CF64
; SC126_CF128     RC    18.432   0xC0   SC126 CF128
...
; Initialise Z180's serial ports
; Assumed to already be initialised by Small Computer Monitor
...
#IFDEF      S5_Original
            .TEXT "CP/M BIOS 1.4 for Z180 with SCM S5 based on code by G. Searle"
#ENDIF
#IFDEF      SC111_CF64
            .TEXT "Z180 BIOS 1.0 for SC111 CF64  based on code by G. Searle"
#ENDIF
#IFDEF      SC111_CF128
            .TEXT "Z180 BIOS 1.0 for SC111 CF128 based on code by G. Searle"
#ENDIF
#IFDEF      SC126_CF64
            .TEXT "Z180 BIOS 1.0 for SC126 CF64  based on code by G. Searle"
#ENDIF
#IFDEF      SC126_CF128
            .TEXT "Z180 BIOS 1.0 for SC126 CF128 based on code by G. Searle"
#ENDIF
#IFDEF      RC_Z180_Native
            .TEXT "CP/M BIOS 1.3 for Z180-RC based on code by G. Searle"
#ENDIF
            .DB CR,LF
            .DB CR,LF
            .TEXT "CP/M 2.2 "
...
;------------------------------------------------------------------------------------------------
; Input device status
; Returns its status in A; 0 if no character is ready, 0FFh if one is available.
...
constA:     IN0  A,(STAT0)      ;Read serial port status register
            BIT  ST_RDRF,A      ;Receive register full?
            JR   Z,constNone    ;Skip if no character available
            LD   A,0xFF         ;Character is available
            RET

constB:     IN0  A,(STAT1)      ;Read serial port status register
            BIT  ST_RDRF,A      ;Receive register full?
            JR   Z,constNone    ;Skip if no character available
            LD   A,0xFF         ;Character is available
            RET
...
constNone:
            LD   A,0            ;Character not available
            RET


;------------------------------------------------------------------------------------------------
; Character input
; Wait until a character is available, then return it in A.
...
coninA:     IN0  A,(STAT0)      ;Read serial port status register
            BIT  ST_RDRF,A      ;Receive register full?
            JR   Z, coninA      ;No, so keep waiting
            IN0  A,(RDR0)       ;Read data byte
            RET                 ;Return character in A

coninB:     IN0  A,(STAT1)      ;Read serial port status register
            BIT  ST_RDRF,A      ;Receive register full?
            JR   Z, coninB      ;No, so keep waiting
...
;------------------------------------------------------------------------------------------------
; Character output
; Wait for device to be ready, then output character in C
...
conoutA:    IN0  A,(STAT0)      ;Read serial port status register
            BIT  ST_TDRE,A      ;Transmit register empty?
            JR   Z,conoutA      ;No, so keep waiting
            OUT0 (TDR0), C      ;Write byte to serial port
            RET

conoutB:    IN0  A,(STAT1)      ;Read serial port status register
            BIT  ST_TDRE,A      ;Transmit register empty?
            JR   Z,conoutB      ;No, so keep waiting
