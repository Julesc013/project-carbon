...
This directory contains the **eZ90 P7 (TURBO_UNLIMITED)** core scaffold: a modular, OoO-ready RTL skeleton intended to be stable for long-term evolution. The v1 scaffold focuses on clean module boundaries and a working simulation-elaboratable top-level, not on finishing the ISA or microarchitecture.

## Top-Level

- `hdl/cores/eZ90/rtl/ez90_core.sv`: eZ90 core top module.
  - Ports:
...
`ez90_core.sv` implements the core-common CSRs required for bring-up:
...
- Replace `fe_decode` with a real decoder emitting `ez90_uop_t` fields.
