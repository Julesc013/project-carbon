...
- **P2 (am9513 native)**: Native CAI submission/completion, multi-context execution, register-file fast path.
...
Legacy mode behavior is intended to remain stable across Am9513 revisions; the native CAI path may grow features over time.
...
- `opcode_group = CAI_OPGROUP_SCALAR`
- `opcode[31] = 1`
- `opcode[7:0]  = function` (Am9513 function selector)
- `format_primary = dst_format` (one of `CARBON_FMT_*`)

Conversions:

- Use `function = AM9513_FUNC_CONV`.
- `format_primary` is destination format, and `submit_desc.flags[7:0]` is source format.
...
### Register-file fast path (P2)

Native mode supports a fast path to write the primary result into `F0..F15`:
...
## 6. Identification and status reporting

`CARBON_CSR_AM9513_ID` returns:

- `[15:0]` device ID (`0x9513`)
- upper bits carry the feature bit mask (uses `CARBON_AM951*_MASK` values)

`CARBON_CSR_AM9513_STATUS` reports:

- `[0]` enabled
- `[1]` CAI busy
- `[2]` legacy busy
- `[15:8]` pending descriptor count
- `[23:16]` presented tier (P2)
- `[31:24]` max supported tier (implementation-defined)

## 7. Legacy compatibility shells (CSR)
...
## 9. Deferred / TODO (explicitly not implemented)
