# 8097 FPU v1.0 (P0 8087-style + P7 native scaffold)

## Overview

`fpu_8097` is a fabric-attached floating-point unit that exposes an **x87-like stack model** (8-deep) plus a minimal **P7 native regfile mode**. v1.0 is controlled via a CSR window (no full x87 instruction decode in this prompt).
...
- Supported tiers in v1.0:
  ...
  - **P7**: supported (enables native RF mode when `MODEFLAG_STRICT==0`)
  - **P1–P6**: not supported; tier writes fault

Native extension access is gated by:
...
## P7 native regfile operations (CARBON_CSR_8097_RF_OP)
...
- CAI integration and async queueing (optional accelerator feature)
